    ...
    with tf.variable_scope(name):
        for i in range(0, count):
            with tf.variable_scope('blk/' + str(i)):
                ...
                l = tf.concat([l, x], axis=3)
        l = BNReLU('blk_bna', l)
    return l
    
############################################
### THIS IS FOR SQUEEZE AND EXCITATION
############################################
...
def atrous_spatial_pyramid_pooling(x, filters=64):
  ...
  with tf.variable_scope('ASSP_layers'):
    ...
    image_level_features = FixedUnPooling('unp_image_level_features', image_level_features, shape=(x.shape[1].value, x.shape[2].value), data_format='channels_last') 
    ...
    net = tf.image.resize_nearest_neighbor(net,[80,80])
    return net


def decoder(name, i, use_assp=True):
    pad = 'same' # to prevent boundary artifacts
    with tf.variable_scope(name):
...
class Model_NP_HV(Model):
    def _build_graph(self, inputs):
        ...
        def loss_msge(true, pred, focus, name=None):
            #MSE of gradients as per paper (unchanged):
